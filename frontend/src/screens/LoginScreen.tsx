    ...
    try {
      ...
      if (result.success) {
        ...
        if (hasSession) {
          ...
          setVerifying(false);
          setError("No active session. Please verify your RIN first.");
  ...
  const verifyPin = async (enteredPin: string) => {
    ...
    try {
      ...
      if (!hasSession) {
        setVerifying(false);
        setError("No active session. Please verify your RIN first.");
        setTimeout(() => navigation.navigate("Identification"), 1500);
        return;
      }

      // Accept PIN if it matches stored or if no PIN set yet (first login after RIN)
