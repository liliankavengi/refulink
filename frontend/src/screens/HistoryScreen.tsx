...
import { getTransactions, type Transaction as ApiTransaction } from "../services/walletService";
...
export default function TransactionHistoryScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const apiTxns = await getTransactions(50);
        const mapped: Transaction[] = apiTxns.map((txn, idx) => ({
          id: idx + 1,
          type: txn.direction === "incoming" ? "received" as TransactionType : "sent" as TransactionType,
          category: txn.direction === "incoming" ? "received" as FilterCategory : "sent" as FilterCategory,
          title: txn.direction === "incoming" ? "RECEIVED" : "SENT",
          source: txn.counterparty_address
            ? `${txn.counterparty_address.slice(0, 6)}...${txn.counterparty_address.slice(-4)}`
            : "Unknown",
          amount: `${txn.direction === "incoming" ? "+" : "-"}KES ${txn.amount_kes.toLocaleString(undefined, { minimumFractionDigits: 2 })}`,
          date: txn.timestamp,
          verified: txn.status === "completed" || txn.status === "success",
          icon: txn.direction === "incoming" ? IconTrendingUp : IconTrendingDown,
          status: txn.status || "Completed",
          reference: txn.tx_hash ? `STL-${txn.tx_hash.slice(0, 6).toUpperCase()}` : undefined,
        }));
        setAllTransactions(mapped);
      } catch (e) {
        console.warn("Failed to fetch transactions:", e);
        // Leave empty — UI will show "no transactions"
      } finally {
        setIsLoading(false);
      }
    })();
