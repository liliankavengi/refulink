...
import { getBalance, getTransactions, type BalanceResponse, type Transaction } from "../services/walletService";
import { getVouchStatus, type VouchStatusResponse } from "../services/verificationService";
...
<<<<<<< HEAD:frontend/src/screens/DashboardScreen.tsx
// Helper to format balance amount
const formatKES = (amount: number): string => {
  return `KES ${amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
};

// Helper to map trust score to letter grade
...
=======
>>>>>>> ec8afc052509ccd940ddfcdba5de9afd1968205d:refu-link-frontend/src/screens/DashboardScreen.tsx
export default function DashboardScreen() {
  ...
  const [vouchStatus, setVouchStatus] = useState<string>("unverified");
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      setBalance(bal.kes_balance);
      setStellarAddress(bal.stellar_address);
    } catch (e) {
      console.warn("Balance fetch failed, using cached/demo:", e);
    }

    // Fetch recent transactions
    try {
      const txns = await getTransactions(5);
      setRecentTxns(txns);
    ...
    // Fetch vouch status
    try {
      const vs = await getVouchStatus();
      setVouchStatus(vs.verification_status);
    } catch (e) {
      console.warn("Vouch status fetch failed:", e);
    }

    // Fetch trust score
    try {
      const ts = await getTrustScore();
      setTrustScore(ts.trust_score);
      setVouchCount(ts.vouches);
      setTxnCount(ts.transactions_count);
    } catch (e) {
      console.warn("Trust score fetch failed:", e);
    }
  }, []);
              ...
              >
                ...
                <View style={styles.balanceFooter}>
                  <View style={styles.verifiedBadge}>
                    <IconCheck color={vouchStatus === 'vouched' ? GREEN : ORANGE} size={14} />
                    <Text style={styles.verifiedText}>
                      {vouchStatus === 'vouched' ? 'Verified' : vouchStatus === 'pending' ? 'Pending' : 'Unverified'}
                    </Text>
                  </View>

                  <Pressable style={styles.scoreButton}>
                    <Text style={styles.scoreText}>Score: {scoreToGrade(trustScore)}</Text>
...
<<<<<<< HEAD:frontend/src/screens/DashboardScreen.tsx
          ...
          <Animated.View style={[
            ...
            <View style={styles.statCard}>
              ...
              <Text style={styles.statValue}>{vouchCount}</Text>
            ...
            <View style={styles.activityList}>
              {recentTxns.length > 0 ? (
                recentTxns.slice(0, 3).map((txn, idx) => (
                  <View key={txn.tx_hash || idx} style={styles.activityItem}>
                    ...
                    <View style={styles.activityInfo}>
                      <Text style={styles.activityTitle}>
                        {txn.direction === 'incoming' ? 'Received' : 'Sent'}
                      </Text>
                    ...
                    <Text style={[
                      ...
                      txn.direction === 'outgoing' && styles.activitySent
                    ]}>
                      {txn.direction === 'incoming' ? '+' : '-'}KES {txn.amount_kes.toLocaleString()}
                    </Text>
                  </View>
                ))
              ) : (
