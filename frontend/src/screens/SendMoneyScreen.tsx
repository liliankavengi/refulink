...
import { getBalance, type BalanceResponse } from "../services/walletService";
...
export default function SendMoneyScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn("Balance fetch failed on SendMoney");
      }
    })();
  }, []);
  ...
  const handleRecipientChange = (value: string) => {
    ...
    // Validate as a Stellar address (G... 56 chars)
    if (value.length >= 56 && value.startsWith("G")) {
      setIsSearching(true);
      
      // Short delay for UX feel, then validate format
      setTimeout(() => {
    ...
    } else if (value.length >= 8 && !value.startsWith("G")) {
      // Treat as a name/identifier lookup — show as pending
      setIsSearching(true);
      setTimeout(() => {
        setRecipientFound(true);
        setIsSearching(false);
  ...
  const handleContinue = () => {
    ...
    if (amountNum > balance) {
      Alert.alert("Insufficient Balance", `Your balance is KES ${balance.toLocaleString()}. Please enter a lower amount.`);
      return;
    }
