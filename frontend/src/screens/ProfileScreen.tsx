...
import { getSession, clearSession, type UserSession } from "../services/authService";
import { getVouchStatus } from "../services/verificationService";
import { getTrustScore } from "../services/trustService";
...
export default function ProfileScreen() {
  ...
  const [vouchStatus, setVouchStatus] = useState("unverified");
  const [txnCount, setTxnCount] = useState(0);
  const [vouchCount, setVouchCount] = useState(0);
  const [trustGrade, setTrustGrade] = useState("--");
  const [userRin, setUserRin] = useState("");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (session) {
          setUserName(session.fullName || "User");
          setUserRin(session.idNumber || "");
        }
      } catch {}
      ...
      try {
        ...
        const score = ts.trust_score;
        if (score >= 90) setTrustGrade("A+");
        else if (score >= 80) setTrustGrade("A");
        else if (score >= 70) setTrustGrade("B+");
        else if (score >= 60) setTrustGrade("B");
        else if (score >= 50) setTrustGrade("C+");
        else setTrustGrade("C");
      } catch {}
    })();
  }, []);
              ...
              >
                <View style={styles.userCardContent}>
                  ...
                  <View style={styles.rinContainer}>
                    ...
                    <Text style={styles.rinNumber}>{userRin || "Not verified"}</Text>
                  </View>

                  {/* Status */}
                  <View style={styles.statusBadge}>
                    <IconCheckCircle color={vouchStatus === 'vouched' ? GREEN : ORANGE} size={16} />
                  ...
                  <View style={styles.statsRow}>
                    ...
                    <View style={styles.statItem}>
                      <Text style={styles.statNumber}>{vouchCount}</Text>
                      <Text style={styles.statLabel}>Vouches</Text>
                    </View>
                    <View style={styles.statDivider} />
                    <View style={styles.statItem}>
                      <Text style={styles.statNumber}>{trustGrade}</Text>
