...
<<<<<<< HEAD:frontend/src/screens/IdentificationScreen.tsx
    ...
    try {
      // Step 1: Verify the RIN/Alien ID via backend
      const verifyResult = await verifyRIN(rin.trim());

      if (!verifyResult.verified) {
        setErrorMsg(verifyResult.message || "Verification failed. Please check your RIN.");
        setIsLoading(false);
        return;
      }

      // Step 2: Register Stellar identity (server generates keypair)
      try {
        await registerIdentity();
      } catch (regError: any) {
        // 409 = already registered, which is fine
        if (regError?.response?.status !== 409) {
          console.warn("Identity registration skipped:", regError?.message);
        }
      }
